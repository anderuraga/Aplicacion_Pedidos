...
$update = urldecode($_GET['lastupdate']);
?>
...
var settingsObject = {
    ...
    settings: {
        layout: {
            ...
            boxed: true,
            logo: {
                darkLogo: 'src/assets/img/logo/<?= $_GET['logo'] ?>',
                lightLogo: 'src/assets/img/logo/<?= $_GET['logo'] ?>'
            }
        }
    },
    reset: false,
...
if (localStorage.length === 0) {
    ...
    if (getcorkThemeObject !== null) {
        ...
        if(!ParsedObject.updated){
            corkThemeObject = settingsObject;
        }else{
            let localdate = new Date(ParsedObject.updated);
            let updatdate = new Date("<?= $update ?>");

            if(updatdate > localdate){
                corkThemeObject = settingsObject;
            }else{
